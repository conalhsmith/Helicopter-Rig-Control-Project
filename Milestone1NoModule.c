...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (!zero_ref)
        {
            helicopterLanded = ((2 * sum + BUF_SIZE) / (2 * BUF_SIZE));
            zero_ref = true;
        }
        ...
        if (state == 0)
        {
            currentAltitude = helicopterLanded - ((2 * sum + BUF_SIZE) / (2 * BUF_SIZE));

            int32_t altitudePercentage = ((int32_t)100 * currentAltitude) / ((int32_t)1241);

            // Display the altitude value on the Orbit OLED display

             char altitudeString[17];
             usnprintf(altitudeString, sizeof(altitudeString), "altitude: %5d%%", altitudePercentage);
             OLEDStringDraw(altitudeString, 0, 1);
        }

        if (state == 1)
        {
            int32_t meanADC = ((2 * sum + BUF_SIZE) / (2 * BUF_SIZE));
            char altitudeString[17];
            usnprintf(altitudeString, sizeof(altitudeString), "mean ADC: %5d   ", meanADC);
            OLEDStringDraw(altitudeString, 0, 1);

        }


        SysCtlDelay(SysCtlClockGet() / 18);  // Update display at ~ 2 Hz
    }
}
